...
public class Torrent {
	...
	public static class TorrentFile {
		...
		public TorrentFile(String filePath, long size) {
	...
	public Torrent(byte[] torrent, boolean seeder) throws IOException {
		...
		if (this.decoded_info.containsKey("files")) {
			for (BEValue file : this.decoded_info.get("files").getList()) {
				...
				this.files.add(
						new TorrentFile(this.name + path.toString(),
	...
	public boolean removeFileByFilename(String filename) {
		for (int i = 0; this.files.size() != i; i++) {
			if (this.files.get(i).filePath.equals(filename)) {
				this.files.remove(i);
				return true;
			}
		}
	...
	public void setFilename(String oldFilename, String newFilename) {
		for (TorrentFile file : this.files) {
			if (file.filePath.equals(oldFilename)) {
