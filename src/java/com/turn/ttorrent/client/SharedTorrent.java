...
public class SharedTorrent extends Torrent implements PeerActivityListener {
	...
	public SharedTorrent(Torrent torrent, File destDir, boolean seeder)
		throws FileNotFoundException, IOException {
		this(torrent.getEncoded(), destDir, seeder, DEFAULT_REQUEST_STRATEGY, true);
	}
	...
	public SharedTorrent(Torrent torrent, File destDir, boolean seeder,
		...
		throws FileNotFoundException, IOException {
		this(torrent.getEncoded(), destDir, seeder, requestStrategy, true);
	}

	public SharedTorrent(Torrent torrent, File destDir, boolean seeder,
						 RequestStrategy requestStrategy, boolean createFiles)
			throws FileNotFoundException, IOException {
		this(torrent.getEncoded(), destDir, seeder, requestStrategy, createFiles);
	}
	...
	public SharedTorrent(byte[] torrent, File parent, boolean seeder)
		throws FileNotFoundException, IOException {
		this(torrent, parent, seeder, DEFAULT_REQUEST_STRATEGY, true);
	}

	public SharedTorrent(byte[] torrent, File parent, boolean seeder, boolean createFile)
			throws FileNotFoundException, IOException {
		this(torrent, parent, seeder, DEFAULT_REQUEST_STRATEGY, createFile);
	}
	 ...
	 */
	public SharedTorrent(byte[] torrent, File parent, boolean seeder,
			RequestStrategy requestStrategy, boolean createFile)
		throws FileNotFoundException, IOException {
		...
		if (createFile) {
			...
			for (Torrent.TorrentFile file : this.files) {
				File actual = new File(parent, file.filePath);
	...
	public static SharedTorrent fromFile(File source, File parent)
		...
		return new SharedTorrent(data, parent, true);
	}

	public static SharedTorrent fromFile(File source, File parent, boolean createFiles)
			throws IOException {
		byte[] data = FileUtils.readFileToByteArray(source);
		return new SharedTorrent(data, parent, true, createFiles);
	}

	public void createTorrentFiles(File parent) throws IOException {
		String parentPath = parent.getCanonicalPath();
